...
- **Case-insensitive AppProject matching**: access is granted by AppProject naming convention (`<team>-XXX-YYY`), and a team's object pattern in `policy.csv` (e.g. `alpha1-*`) is matched against the AppProject name case-insensitively, so `Alpha1-Web` and `ALPHA1-API` are both visible to a team granted `alpha1-*`.
